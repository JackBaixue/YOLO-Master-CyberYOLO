"""
（ICCV 2023）Rethinking Vision Transformers for MobileNet Size and Speed
论文地址：https://arxiv.org/abs/2212.08059
论文代码：https://github.com/snap-research/EfficientFormer

Q: 这篇论文试图解决什么问题？

A:  这篇论文试图解决如何设计和优化视觉变换器（Vision Transformers，简称ViTs），使其能够在保持高性能的同时，达到与MobileNet相当的模型大小和推理速度。具体来说，论文关注的核心问题包括：
        1. 模型大小和速度的优化：尽管Vision Transformers在计算机视觉任务中取得了显著的性能，但它们通常比轻量级卷积神经网络（CNNs）如MobileNet具有更高的延迟和更多的参数。
            这限制了ViTs在资源受限的移动设备上的广泛应用。因此，论文探索了如何设计一个既能保持高性能又能实现MobileNet级别大小和速度的Transformer模型。
        2. 高效的架构搜索策略：为了找到在模型大小和速度上都高效的Transformer架构，论文提出了一种新的细粒度联合搜索策略，该策略能够同时优化模型的延迟和参数数量，从而找到在这些约束条件下的最优架构。
        3. 移动友好型设计选择：论文重新审视了ViTs的设计选择，并提出了一系列针对移动设备优化的架构改进，这些改进旨在减少计算复杂度和延迟，同时保持或提升模型性能。
