...
论文代码：https://github.com/microsoft/Cream/tree/main/EfficientViT

Q: 这篇论文试图解决什么问题？

A:  这篇论文试图解决的是Vision Transformers（ViTs，即视觉变换器）在实际应用中的效率问题。
    尽管Vision Transformers因其高模型能力在计算机视觉领域取得了巨大成功，但它们的显著性能提升伴随着模型尺寸的增加和计算开销的上升，这使得它们在需要实时响应的应用中变得不太适用。
    具体来说，论文中提到的问题包括：
        1. 计算成本高昂：现有Transformer模型的计算成本很高，这限制了它们在实时应用中的使用。
        2. 内存效率低：Transformer模型中某些操作（如多头自注意力MHSA中的张量重塑和逐元素函数）内存效率低，这导致实际推理吞吐量受限。
        3. 计算冗余：多头注意力机制中存在注意力头学习相似线性投影的现象，导致计算资源的浪费。
        4. 参数使用效率低：现有轻量级模型在参数分配上通常遵循标准Transformer模型的配置，没有针对提高参数效率进行优化。
    为了解决这些问题，论文提出了一系列改进措施，包括设计了新的构建模块、提出了级联组注意力模块（Cascaded Group Attention, CGA），
    以及对参数进行重新分配，旨在提高Vision Transformers的内存效率、计算效率和参数效率，从而在保持或提升性能的同时，显著提高模型的运行速度。
